...
export default function BookingAppointment() {
    ...
    useEffect(() => {
        ...
        const fetchDoctorInfo = async () => {
            try {
                ...
                setDoctorInfo(data); // Set doctorInfo state with fetched data
            } catch (error) {
    ...
    useEffect(() => {
        const fetchDoctorSchedule = async () => {
            try {
                const response = await fetch(`http://localhost:9999/doctorSchedule`);
                if (!response.ok) {
                    throw new Error('Failed to fetch doctor schedule');
                }
                const data = await response.json();
                console.log("Data from API:", data); // Kiểm tra dữ liệu nhận được từ API
    
                const filteredSchedule = data.filter(schedule => schedule.doctorId === id);
                console.log("Filtered Schedule:", filteredSchedule); // Kiểm tra dữ liệu sau khi filter
    
                setDoctorSchedule(filteredSchedule); // Cập nhật state với dữ liệu đã lọc
                console.log("Doctor Schedule after update:", doctorSchedule); // Kiểm tra giá trị mới của doctorSchedule
            } catch (error) {
    ...
    const handleBooking = async () => {
        try {
            ...
            const appointmentData = {
                patientId,
                doctorId: id,
                appointmentTime: `${selectedDate} ${selectedSlot}`,
                status: "true",
                        ...
                        <div className="w-full border rounded-md shadow-md p-4 bg-white">
                            ...
                            <div className="w-full flex px-[20px] gap-[20px] my-[20px]">
                                <div className="bg-slate-500 w-[50px] h-[50px] rounded-[40px]">
                                    <img src={doctorInfo?.avatar} alt="doctor" className="rounded-full" />
                                </div>
                                <div className="">
                            ...
                            <div className="w-full flex my-[20px]">
                                ...
                                <div className="w-1/2">
                                    ...
                                    <p className="text-[18px]">{patientInfo?.fullname}</p>
                                </div>
                            </div>
